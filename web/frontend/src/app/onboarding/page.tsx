...
export default function OnboardingPage() {
    ...
    createClient()
      ...
      .then(({ data }) => {
        const fullName =
          ...
          "bạn";
        setUserName(fullName.split(" ").pop() || fullName);
      });
  }, []);
  ...
  // Auto-generate AI messages after analysis lands
  useEffect(() => {
    if (!analysis) return;
    ...
    const prioritiesText = analysis.top_priorities
      .map((p, i) => `${i + 1}. ${p}`)
      .join("\n");
    ...
    const sectionSummary = weakSections.length > 0
      ? weakSections
  ...
  // When user selects a section, add AI message about it
  useEffect(() => {
  ...
  async function sendMessage() {
    ...
    setMessages((m) => [...m, { id: "streaming", role: "ai", content: "" }]);
      ...
      <div className="min-h-screen flex flex-col items-center justify-center px-6 py-12">
        ...
        <div className="w-full max-w-lg">
          ...
          <div className="flex gap-3 mb-8">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-brand-500 to-indigo-600 flex items-center justify-center text-white shrink-0 mt-1 shadow-md">
              <Sparkles className="w-5 h-5" />
            </div>
            <div className="bg-white rounded-2xl rounded-tl-sm px-5 py-4 shadow-card border border-slate-100 flex-1">
              <p className="text-slate-800 text-sm leading-relaxed">
                Chào {userName || "bạn"}! 👋 Tôi là <strong>Vica AI</strong>, trợ lý tìm việc
                của bạn.
              </p>
              <p className="text-slate-800 text-sm leading-relaxed mt-2">
                Để bắt đầu hiệu quả, hãy upload CV của bạn. Tôi sẽ{" "}
                <strong>phân tích chi tiết từng mục</strong> và chỉ ra cụ thể những gì cần cải
                thiện nhé!
              </p>
            </div>
          </div>
          ...
          >
            ...
            <p className="font-semibold text-slate-700 text-sm">Kéo thả CV vào đây</p>
            <p className="text-slate-400 text-xs mt-1">hoặc click để chọn file</p>
            <p className="text-slate-300 text-xs mt-3">Hỗ trợ PDF · DOC · DOCX · TXT</p>
          </div>
          <input
            ref={fileInputRef}
            type="file"
          ...
          <div className="text-center mt-6">
            <button
              ...
              className="text-slate-400 text-xs hover:text-slate-600 transition-colors cursor-pointer"
            >
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6">
        ...
        <div className="w-48 h-1.5 bg-slate-100 rounded-full overflow-hidden">
          <div
            className={clsx(
              "h-full bg-brand-500 rounded-full transition-all duration-700",
  ...
  return (
    <div className="h-screen flex flex-col">
      {/* Header */}
      <header className="h-14 border-b border-slate-100 bg-white flex items-center justify-between px-6 shrink-0 z-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={completeOnboarding}
            className="flex items-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-all duration-150 cursor-pointer shadow-btn-cta"
          >
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Left: CV sections */}
        <div className="w-[55%] border-r border-slate-100 overflow-y-auto bg-slate-50/50 p-6 scrollbar-thin">
              ...
              {analysis.sections.map((section: CVSection) => {
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      "w-full text-left bg-white rounded-2xl border transition-all duration-200 overflow-hidden cursor-pointer",
                      isActive
                  ...
                  >
                    ...
                    {/* Expanded detail */}
                    {isActive && (
                      <div className="border-t border-slate-100 px-4 pb-4 pt-3 bg-slate-50/60 text-left">
                        {section.issues.length > 0 && (
                          <div className="mb-3">
          ...
          <div className="border-t border-slate-100 p-4 bg-white shrink-0">
            <div className="flex gap-2">
              <input
                ...
                disabled={streaming}
                className="flex-1 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent disabled:opacity-60"
              />
              <button
                onClick={sendMessage}
                disabled={streaming || !chatInput.trim()}
                className="bg-brand-500 hover:bg-brand-600 disabled:opacity-40 text-white w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-150 cursor-pointer shrink-0"
              >
